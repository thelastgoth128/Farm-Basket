...
import { ProductsService } from 'src/components/products/products.service';
...
export class ReviewsService {
  ...
  async updateReview(userId: number, reviewId: number, updateReviewDto: UpdateReviewDto): Promise<Review> {
    const review = await this.reviewsRepository.findOne({ where: { id: reviewId, user: { userid: userId } } });
    if (!review) {
  ...
  async getProductReviews(productId: number): Promise<Review[]> {
    return this.reviewsRepository.find({ where: { product: { productid: productId } }, relations: ['user'] });
  }
}